...
public class Bush extends Species {
    ...
    public void setup() {
        Block logBlock = BuiltInRegistries.BLOCK.getValue(log);
        Block leavesBlock = BuiltInRegistries.BLOCK.getValue(leaves);
        Block altLeavesBlock = null;
        if (altLeaves != null) {
            Block altLeafBlock = BuiltInRegistries.BLOCK.getValue(altLeaves);
